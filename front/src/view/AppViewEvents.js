...
export function initAppEvents(view) {
    window.addEventListener("resize", () => {
        ...
        const activeIndex = icons.findIndex((i) =>
    ...
    document.addEventListener("click", (e) => {
        ...
        if (switcher?.classList.contains("show")) {
            if (
                !e.target.closest(".account-switcher-sheet") &&
                !headerProfile
            ) {
        ...
        if (bottomMenu?.classList.contains("show")) {
            ...
            const isMenuIcon =
                clickedIcon && clickedIcon.dataset?.page === "menu";
        ...
        if (paramBtn) {
            paramBtn.classList.add("spinning");
            setTimeout(() => paramBtn.classList.remove("spinning"), 500);
        ...
        if (footerIcon) {
            ...
            if (
                document.body.classList.contains("parent-mode") &&
                page === "home"
            ) {
                page = "parent-home";
            }
    ...
    view.appRoot.addEventListener("click", (e) => {
        const pathBtn = e.target.closest(".path-button-container");

        if (pathBtn && !e.target.closest(".duo-popup")) {
            const step = pathBtn.closest(".path-step");
